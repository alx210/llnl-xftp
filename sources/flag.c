...
{
	/* Create flag pixmaps */
	okflag_pixmap = create_pixmap(&okflag);
	errorflag_pixmap = create_pixmap(&errorflag);

	/* Initialize result flag to "OK" */
    XtVaSetValues(w_resultFlag,
...
raise_okflag()
{
	...
	XtVaGetValues(w_resultFlag, XmNuserData, &okflag_raised, NULL);
	
    if (!okflag_raised)
        XtVaSetValues(w_resultFlag,
			XmNlabelPixmap, okflag_pixmap,
			XmNuserData,	(long)True,
			NULL
		);
}
